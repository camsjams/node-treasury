...
test('memcached adapter should construct', () => {
	...
	// act
	const result = new MemcachedAdapter(client);
...
test('memcached adapter should get [rejects from Error]', async () => {
	// arrange
	expect.assertions(1);
	const adapter = new MemcachedAdapter({get: sinon.stub().yields(new Error('invalid database'))});
...
test('memcached adapter should get [rejects from undefined value]', async () => {
	// arrange
	expect.assertions(1);
	const adapter = new MemcachedAdapter({get: sinon.stub().yields(undefined, undefined)});
...
test('memcached adapter should get [resolves]', async () => {
	...
	const adapter = new MemcachedAdapter({get: sinon.stub().withArgs(INPUT).yields(undefined, '{"dogs":true}')});
...
test('memcached adapter should set [rejects from Error]', async () => {
	...
	const adapter = new MemcachedAdapter({set: sinon.stub().yields(ERROR)});
...
test('memcached adapter should set [resolves]', async () => {
	// arrange
	expect.assertions(1);
	const adapter = new MemcachedAdapter({set: sinon.stub().withArgs('dogs', 101, 'dalmatians').yields(undefined)});
...
test('memcached adapter should del [rejects from Error]', async () => {
	// arrange
	expect.assertions(1);
	const adapter = new MemcachedAdapter({del: sinon.stub().yields(new Error('invalid database'))});
...
test('memcached adapter should del [rejects from null value]', async () => {
	// arrange
	expect.assertions(1);
	const adapter = new MemcachedAdapter({del: sinon.stub().yields(undefined, null)});
...
test('memcached adapter should del [resolves]', async () => {
	// arrange
	expect.assertions(1);
	const adapter = new MemcachedAdapter({del: sinon.stub().withArgs('dogs').yields(undefined)});
